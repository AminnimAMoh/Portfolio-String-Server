...
const reDrawCan = (annualrain, months) => {
    ...
    legendGraph.attr("transform", "translate(" + (265 + mapXOffSet) + " ," + (h - 100) + ")");
    ellipseContainer
        .selectAll("ellipse")
        .data(ellipsesLength)
        .enter()
    ...
    let circleStations = cityCircles.selectAll('circle')
        ...
        for (let i = 0; i < 3; i++) {
            ...
            let groupTx = mapXOffSet + coords[0] + graphRad * Math.cos(angleScale(i));
            let groupTy = coords[1] + graphRad * Math.sin(angleScale(i));
            groupContainer.attr("transform", "translate(" + groupTx + "," + groupTy + ")");
        }
        let rainGroupTx = mapXOffSet + coords[0] + graphRad * Math.cos(angleScale(3)) - 125;
        ...
        let margin = {
            ...
            bottom: 50,
            left: 80,
        }, width = Math.min(700, window.innerWidth / 4) - margin.left - margin.right, height = Math.min(width, window.innerHeight - margin.top - margin.bottom);
        // let data = [];
        let thisCityRain = [];
        ...
        textContainer
            ...
            .attr("font-size", 11)
            .style("opacity", 1);
        let groupTx = mapXOffSet + coords[0] + graphRad * Math.cos(angleScale(0));
